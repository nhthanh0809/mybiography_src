...
* We propose a state-of-the-art post-hoc CNN specific 
  Visual XAI algorithm - SISE.
* Input      <-  An image; The trained model
* Output     ->  A visual heatmap 
* Properties :   Noise-free, High resolution, Class discriminative and
                 Correlates model's prediction
```
...
systems established based on such models.
...
We introduce a novel XAI algorithm that offers both **spatial resolution** and **explanation completeness** in its output explanation map by 
...
<!--
**Ideas**
1. Visualizing multiple layers of the network (improves spatial resolution).
2. Mutual utilization of perturbation and backpropagation for visualizing the layers (performance-speed trade-off).
3. Selecting the minimum number of layers to visualize (performance-speed trade-off).
4. Employing a framework similar to RISE, but with attribution masks instead of random masks (this makes our method CNN-specific).
-->

Our proposed algorithm is motivated by ‘perturbation-based’ XAI methods that attempt to interpret the model’s prediction by employing input sampling techniques. These methods have shown a great faithfulness in rationally inferring the predictions of models. However, they show instability as their output depends on random sampling (RISE) [3] or random
initialization for optimizing a perturbation mask (Extremal perturbation) [4]. They also produce different explanation maps each time and have an excessive runtime when attempting to get generalized results.
...
<p align="center">
    <img src="docs/SISE_architecture.svg" alt="SISE_architecture" />
    <br>
...
<p align="center">
    <img src="docs/pooling_screenshot.JPG" width="500" height="275" alt="pooling_screenshot" />
    <br>
